...
export default function Progreso() {
  ...
  useEffect(() => {
    if (profile?.id) cargarEjercicios()
  }, [profile?.id])
  ...
  const cargarEjercicios = async () => {
    try {
      // Traer todas las sesiones completadas del socio
      ...
      // Traer ejercicios únicos registrados
  ...
  const cargarRegistros = async (ejercicioId) => {
    ...
    try {
      ...
      // Agrupar por sesión: tomar el peso máximo
      const porSesion = {}
      ;(data || []).forEach((r) => {
    ...
    return (
      <div className="p-6">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-serif font-bold mt-1">Progreso</h1>
        </div>
        <div className="card text-center py-12">
          ...
          <p className="font-medium mb-1">Sin datos todavía</p>
          <p className="text-sm text-text-muted max-w-xs mx-auto">
            Completá al menos un entrenamiento para ver tu progreso por ejercicio.
          </p>
        </div>
      </div>
    )
